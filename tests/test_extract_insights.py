...
class ExtractInsightsTests(unittest.TestCase):
    ...
    def test_signal_score_prefers_recent_issue(self) -> None:
        ...
        recent = self._issue(comments=5, upvotes=3, created_at=now.isoformat(), labels=[], title="same issue")
        old = self._issue(comments=5, upvotes=3, created_at=(now - timedelta(days=365 * 3)).isoformat(), labels=[], title="same issue")
    ...
    def test_signal_score_comments_dominate_after_log_scaling(self) -> None:
        created_at = datetime.now(timezone.utc).isoformat()
        comment_heavy = self._issue(comments=10, upvotes=0, created_at=created_at, labels=[], title="comment heavy")
        upvote_only = self._issue(comments=0, upvotes=1, created_at=created_at, labels=[], title="upvote only")
    ...
    def test_category_assignment(self) -> None:
        ...
        for labels, title, expected in cases:
            with self.subTest(labels=labels, title=title):
                issue = self._issue(comments=0, upvotes=0, created_at=created_at, labels=labels, title=title)
                result = extract_insights._extract_issue(issue, "owner/repo")
